...
myForm.addEventListener("submit", (e) => {
  ...
  const resultServices = getServicesAuth(
    documentData,
    httpMethod,
    endpointUrl,
    token
...
async function populateRoles() {
  const token = appStorage.getItem(KEY_TOKEN) || "";
  try {
    const response = await getServicesAuth(null, METHODS[0], URL_ROLE, token);
    if (!response.ok) throw new Error(`HTTP error! status: ${response.status}`);

    const data = await response.json();

    if (!Array.isArray(data.data)) {
      console.error("Respuesta roles no es un arreglo en 'data':", data);
      return;
    }
    ...
    data.data.forEach(role => {  // Aquí accedes a la propiedad data que es el arreglo
...
document.addEventListener("DOMContentLoaded", populateRoles);
